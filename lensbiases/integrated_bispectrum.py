...
def generate_products(outpath = pathlib.Path("numbaproducts"), 
    ...
    ksaving = np.logspace(-5, 2, Nksaving)
    zsaving = np.append(0, np.logspace(-5, 3, Nzsaving))

    zm = np.logspace(-9, np.log10(1089), Nkzm)
    zm = np.append(0, zm)
    ...
    kgrid = np.log(np.logspace(-5, 2, Nkgrid))
    zgrid = np.append(0, np.logspace(-5, 3, Nzgrid))
    ...
    neff2Dinterp = interp.RectBivariateSpline(zgrid, ksneff, neff2D)
    neff2Dinterpf = lambda z, k: neff2Dinterp(z, k, grid = False)

    logz = np.log10(zsaving)
    logk = np.log10(ksaving)

    functions = [lambda z, k: PK.P(z, k, grid = False), lambda z, k: PKlin.P(z, k, grid = False), lambda z, k: neff2Dinterp(z, k, grid = False)]
